...
def train_epoch(epoch, start_step, global_step, model, optimizer, scaler, train_loader, args, ctx, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if args.save_final_model and (epoch == args.epochs - 1 and step == len(train_loader) - 1):
            model_path = f'{args.save_dir}/sft_final.pth'
            torch.save(model.state_dict(), model_path)
            print(f"Final SFT model saved to {model_path}")
...
def init_model_and_optimizer(args, checkpoint_path=None):
    ...
    if args.pretrained_path and os.path.exists(args.pretrained_path):
        ...
        if args.pretrained_path.endswith('.pth'):
            ...
            pretrained_model = AutoModel.from_pretrained(args.pretrained_path)
...
def main():
    ...
    parser.add_argument("--pretrained_path", type=str, default="./results/pretrain_final.pth", 
                       help="Path to pretrained model (PyTorch .pth file or HF directory)")
